...
/**
*\*\file dual_usbd_cdc_core_loopback.c
*\*\author Nations
*\*\version v1.0.0
*\*\copyright Copyright (c) 2025, Nations Technologies Inc. All rights reserved.
**/

#include "dual_usbd_cdc_core_loopback.h"
#include "usbd_desc.h"
#include "usbd_req.h"
...
extern uint8_t USB1_Rxbuffer[CDC_DATA_MAX_PACKET_SIZE * 8];
extern uint8_t USB2_Rxbuffer[CDC_DATA_MAX_PACKET_SIZE * 8];
...
__ALIGN_BEGIN uint8_t USBD_CDC_OtherCfgDesc  [USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END ;
  ...
  #endif
#endif /* USB_INTERNAL_DMA_ENABLED */
__ALIGN_BEGIN static __IO uint32_t  USB1D_CDC_AltSet  __ALIGN_END = 0;
  ...
  #endif
#endif /* USB_INTERNAL_DMA_ENABLED */
__ALIGN_BEGIN static __IO uint32_t  USB2D_CDC_AltSet  __ALIGN_END = 0;
  ...
  #endif
#endif /* USB_INTERNAL_DMA_ENABLED */
__ALIGN_BEGIN uint8_t USB1_CmdBuff[CDC_CMD_PACKET_SIZE] __ALIGN_END ;
  ...
  #endif
#endif /* USB_INTERNAL_DMA_ENABLED */
__ALIGN_BEGIN uint8_t USB2_CmdBuff[CDC_CMD_PACKET_SIZE] __ALIGN_END ;
...
uint8_t USBD_CDC_Init (void  *pdev, uint8_t cfgidx)
{
    ...
    if ((uint32_t *)(((USB_CORE_MODULE *)pdev)->regs.GCSR) == (uint32_t *)USBHS1_BASE_ADDR)
    {
        USBDEV_EP_PrepareRx(pdev, CDC_OUT_EP, (uint8_t*)(USB1_Rxbuffer), CDC_DATA_OUT_PACKET_SIZE);
    }
    else if ((uint32_t *)(((USB_CORE_MODULE *)pdev)->regs.GCSR) == (uint32_t *)USBHS2_BASE_ADDR)
    {
        USBDEV_EP_PrepareRx(pdev, CDC_OUT_EP, (uint8_t*)(USB2_Rxbuffer), CDC_DATA_OUT_PACKET_SIZE);
    }
    else
    {
        ...
        case USB_REQ_TYPE_CLASS :
            ...
            if (req->wLength)
            {
                ...
                else /* Host-to-Device request */
                {
                    ...
                    if ((uint32_t *)(((USB_CORE_MODULE *)pdev)->regs.GCSR) == (uint32_t *)USBHS1_BASE_ADDR)
                    {
                        USBD_CtrlPrepareRx (pdev, USB1_CmdBuff, req->wLength);
                    }
                    else if ((uint32_t *)(((USB_CORE_MODULE *)pdev)->regs.GCSR) == (uint32_t *)USBHS2_BASE_ADDR)
                    {
                        USBD_CtrlPrepareRx (pdev, USB2_CmdBuff, req->wLength);
                    }
                    else
                    {
        ...
        case USB_REQ_TYPE_STANDARD:
            switch (req->bRequest)
            {
                ...
                case USB_REQ_GET_INTERFACE :
                    if ((uint32_t *)(((USB_CORE_MODULE *)pdev)->regs.GCSR) == (uint32_t *)USBHS1_BASE_ADDR)
                    {
                        USBD_CtrlSendData (pdev, (uint8_t *)&USB1D_CDC_AltSet, 1);
                    }
                    else if ((uint32_t *)(((USB_CORE_MODULE *)pdev)->regs.GCSR) == (uint32_t *)USBHS2_BASE_ADDR)
                    {
                        USBD_CtrlSendData (pdev, (uint8_t *)&USB2D_CDC_AltSet, 1);
                    }
                    else
                    {
                ...
                case USB_REQ_SET_INTERFACE :
                    if ((uint8_t)(req->wValue) < USBD_ITF_MAX_NUM)
                    {
                        if ((uint32_t *)(((USB_CORE_MODULE *)pdev)->regs.GCSR) == (uint32_t *)USBHS1_BASE_ADDR)
                        {
                            USB1D_CDC_AltSet = (uint8_t)(req->wValue);
                        }
                        else if ((uint32_t *)(((USB_CORE_MODULE *)pdev)->regs.GCSR) == (uint32_t *)USBHS2_BASE_ADDR)
                        {
                            USB2D_CDC_AltSet = (uint8_t)(req->wValue);
                        }
                        else
                        {
...
uint8_t  USBD_CDC_DataOut (void *pdev, uint8_t epnum)
{
    ...
    APP_FOPS.pIf_DataRx(pdev, USB_Rx_Cnt);

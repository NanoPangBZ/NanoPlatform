...
#define HPF_SQRT1_2 0.70710678118654752440f

static int hpf_run_freq_allowed(const high_pass_filter_param_t *p)
{
    if (p->run_freq_hz > 0.f) {
        return 1;
    }
    if ((p->flags & HIGH_PASS_FILTER_PARAM_FLAG_IGNORE_RUN_FREQ) == 0u) {
        return 0;
    }
    if (p->order == 1u && p->cutoff_freq_hz <= 0.f) {
        return 1;
    }
    if (p->order == 2u && (p->flags & HIGH_PASS_FILTER_PARAM_FLAG_PRECOMPUTED_BIQUAD) != 0u) {
        return 1;
    }
    return 0;
}

/* 一阶：alpha = 1 / (1 + 2*pi*fc/fs) */
static float hpf_first_order_alpha(float fc_hz, float fs_hz)
{
    ...
    return 1.0f / (1.0f + 2.0f * HPF_PI * fc_hz / fs_hz);
}

/* RBJ 高通双二阶，系数归一化到 a0=1 */
static int hpf_biquad_highpass(float fc_hz, float fs_hz, float Q, float *b0, float *b1,
                               float *b2, float *a1, float *a2)
{
    ...
    const float w0 = 2.0f * HPF_PI * fc_hz / fs_hz;
...
int high_pass_filter_init(high_pass_filter_t *filter, high_pass_filter_param_t *param)
{
    ...
    if (!hpf_run_freq_allowed(param)) {
        return -2;
    }
    if (param->order != 1u && param->order != 2u) {
        return -2;
    }
    if ((param->flags & HIGH_PASS_FILTER_PARAM_FLAG_PRECOMPUTED_BIQUAD) != 0u &&
        param->order != 2u) {
        return -2;
    }
    ...
    if (param->order == 1u) {
        ...
        if (param->cutoff_freq_hz > 0.f) {
            alpha = hpf_first_order_alpha(param->cutoff_freq_hz, param->run_freq_hz);
        ...
        if ((param->flags & HIGH_PASS_FILTER_PARAM_FLAG_PRECOMPUTED_BIQUAD) != 0u) {
            ...
            if (hpf_biquad_highpass(param->cutoff_freq_hz, param->run_freq_hz, Q, &filter->b0,
...
float high_pass_filter_update(high_pass_filter_t *filter, float input)
{
    ...
    if (filter->param.order == 1u) {
        const float alpha = filter->param.alpha;
        const float y = alpha * (filter->prev_output + input - filter->prev_input);

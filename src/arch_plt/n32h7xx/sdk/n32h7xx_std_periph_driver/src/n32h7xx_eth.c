...
void ETH_RxVLANTagInit(ETH_Module* ETHx, ETH_RxVLANTagInitType* ETH_RxVTInitParam)
{
    ...
    TempValue = ((uint32_t)(ETH_RxVTInitParam->VLANTagInStatus)     | (uint32_t)(ETH_RxVTInitParam->VLANTagHashTableMatch)
               | (uint32_t)(ETH_RxVTInitParam->StripVLANTag)        | (uint32_t)(ETH_RxVTInitParam->VLANTypeCheck)
               | (uint32_t)(ETH_RxVTInitParam->VLANTagInverceMatch) | (uint32_t)(ETH_RxVTInitParam->VLANTagComparison12Bit));
...
EthFuncStatusType ETH_BuildRxDescriptors(ETH_Module* ETHx, ETH_InfoType* pInfo)
{
    ...
    if ((ETHx->DMACH0STS & ETH_DMACH0STS_RBU) != (uint32_t)RESET)
    {
        /* Clear RBUS ETHERNET DMA flag */
        ETHx->DMACH0STS = ETH_DMACH0STS_RBU;
...
EthFuncStatusType ETH_PrepareTxDescriptors(ETH_Module* ETHx, ETH_InfoType* pInfo, ETH_TxPacketType* pTxPacket, uint32_t ItMode)
{
    ...
    if ((ETHx->DMACH0STS & ETH_DMACH0STS_TBU) != (uint32_t)RESET)
    {
        /* Clear TBUS ETHERNET DMA flag */
        ETHx->DMACH0STS = ETH_DMACH0STS_TBU;

...
//io操作属性
typedef enum{
    NANO_UNDEFINE_IO = 0x00,        //未定义的IO
    NANO_BIO = 0x01 << 0 ,          //阻塞式IO - 阻塞cpu，调用read/write时不拷贝读/写数据，等待底层读/写完成后才返回
    NANO_NIO = 0x01 << 1 ,          //非阻塞式IO - 不阻塞cpu，调用read/write时读/写内容拷贝到用户/底层缓冲区后立刻返回，底层读/写结果不通知用户
    NANO_AIO = 0x01 << 2 ,          //异步IO - 不阻塞cpu，调用read/write时不拷贝读/写数据到用户/底层缓冲区，调用后立刻返回，读/写完成后通过回调函数通知用户
    NANO_IO_MAP = 0x01 << 3 ,       //内存映射 - 不阻塞cpu，通过对指定的read/write内存段进行操作完成读写，调用read/write后的行为不确定，由实现模块自由决定
    NANO_ALL_IO_MODE = 0xFF,
}nano_io_mode_e;
